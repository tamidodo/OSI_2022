...
for amass in apophis_masses:
    for zangle in vzangles:
        for bspeed in ball_speeds:
            print(
                "bspeed= {}km/s, zangle= {} degrees, amass= {} solar masses".format(
                    bspeed, zangle, amass
                )
            )
            bspeed = (
                bspeed * 86400 * 365.175 / (1.496e8 * 2 * np.pi)
            )  # convert velocity to AU/year divided by 2pi
            sim = rb.Simulation()
            ...
            sim.add(
                m=5.5304172951e-31,  # sat mass is 1.1 kg
                x=xs_init,
                y=ys_init,
                z=zs_init,
                vx=vxs_init,
            ...
            print(
                ...
                "sim.add(m={}, x={}, y={}, z={}, vx={}, vy={}, vz={}, hash='{}')".format(
                    p1.m, p1.x, p1.y, p1.z, p1.vx, p1.vy, p1.vz, names[idSat]
                ),
                "sim.add(m={}, x={}, y={}, z={}, vx={}, vy={}, vz={}, hash='{}')".format(
                    p2.m, p2.x, p2.y, p2.z, p2.vx, p2.vy, p2.vz, names[idLBall]
                ),
                "sim.add(m={}, x={}, y={}, z={}, vx={}, vy={}, vz={}, hash='{}')".format(
                    p3.m, p3.x, p3.y, p3.z, p3.vx, p3.vy, p3.vz, names[idRBall]
                ),
            )
            ...
            # 5 rows, Noutputs columns
            x = np.zeros((5, Noutputs))
            y = np.zeros((5, Noutputs))
            z = np.zeros((5, Noutputs))
            vx = np.zeros((5, Noutputs))
            vy = np.zeros((5, Noutputs))
            vz = np.zeros((5, Noutputs))
            ...
            dfx["rowtype"] = ["xEarth", "xApophis", "xSat", "xLBall", "xRBall"]
            dfy = pd.DataFrame(y)
...
result.to_csv("osi_ball_sim_results.csv", index=False)
