...
vzangles = np.linspace(-40, 0, 21)
vyangle = -5 * twopi / 360
dtfine = [5 * sec2code, -5 * sec2code]
...
d1 = 1000  # distance from the first satellite to Apophis
d2 = 2000  # distance from the first satellite to Apophis
xa_init = -1.052338684548919
ya_init = -0.08311033534942694
za_init = -0.020562053225327438
vxa_init = 0.17920172048583177
vya_init = -0.8794121487974516
vza_init = 0.05119423378893265
...
for amass in apophis_masses:
    for zangle in vzangles:
        for dt in dtfine:
            ...
            sim.dt = dt  # multiply by negative one for backwards integration
            ...
            dfx = pd.DataFrame(x)
            dfx[["rowtype", "dt", "vi_angle", "Apophis_mass"]] = "x", dt, zangle, amass
            dfy = pd.DataFrame(y)
            dfy[["rowtype", "dt", "vi_angle", "Apophis_mass"]] = "y", dt, zangle, amass
            dfz = pd.DataFrame(z)
            dfz[["rowtype", "dt", "vi_angle", "Apophis_mass"]] = "z", dt, zangle, amass
            dfdist1 = pd.DataFrame(distancesat1)
            dfdist1[["rowtype", "dt", "vi_angle", "Apophis_mass"]] = (
                "dist1",
                dt,
                zangle,
                amass,
            ...
            dfdist2[["rowtype", "dt", "vi_angle", "Apophis_mass"]] = (
                "dist2",
                dt,
                zangle,
                amass,
            )
            result = pd.concat([result, dfx, dfy, dfz, dfdist1, dfdist2])


result.to_csv("osi_sat_sim_results.csv", index=False)
